const emotionCards = [
  { key: 'happy', label: 'سعيد', image: 'happy.png' },
  { key: 'sad', label: 'حزين', image: 'sad.png' },
  { key: 'angry', label: 'غاضب', image: 'angry.png' },
  { key: 'funny', label: 'مضحك', image: 'funny.png' },
  { key: 'scared', label: 'خائف', image: 'scared.png' },
  { key: 'sleepy', label: 'نعسان', image: 'sleepy.png' },
  { key: 'tired', label: 'متعب', image: 'tired.png' },
];
...
const MISSIONS = {
  1: ' المهمة: ما هو الشكل المناسب للشعور التالي',
  2: ' المهمة: طابق 5 مشاعر مختلفة يا بطل',
  3: ' المهمة: احفظ الترتيب في 10 ثوانٍ ثم كرره بالترتيب الصحيح',
};

function setLevelUI(level) {
  if (levelBadge) levelBadge.textContent = 'المستوى ' + level;
...
function updateScore() {
  if (scoreBox) {
    scoreBox.textContent = `النقاط: ${gameScore} / 5`;
  }
}
...
function endGame() {
  ...
  if (nextLvlBtn) {
    if (gameLevel === 2) {
      nextLvlBtn.textContent = 'المتابعة إلى المستوى 3 ←';
...
function startLevel3(sequence) {
  ...
  let timeLeft = 10;
  lvl3Countdown.textContent = `يختفي خلال ${timeLeft}ث…`;
...
function showLvl3Step() {
  ...
  if (targetBox) {
    targetBox.innerHTML = `المشاعر <strong>${lvl3CurrentStep + 1}</strong> من ${lvl3Sequence.length} — ما كانت؟`;
  }
  renderLvl3Choices();
}
...
function onLvl3ChoiceSelected(selectedKey, button) {
  ...
  if (selectedKey === expected.key) {
    ...
    if (lvl3CurrentStep >= lvl3Sequence.length) {
      ...
      setTimeout(() => {
        if (lvl3FirstTryScore === lvl3Sequence.length) {
          ...
          if (restartBtn) restartBtn.classList.add('hidden');
          showOverlay(
            ' محاولة جيدة! شاهد الترتيب مرة أخرى وحاول مجدداً!',
            ' حاول مجدداً',
            () => {
    ...
    if (resultBox) {
      resultBox.textContent = '❌ ليس هذا! حاول مرة أخرى.';
...
function onChoiceSelected(selectedKey, button) {
  ...
  if (selectedKey === gameTarget.key) {
    ...
    if (gameLevel === 1) {
      ...
      setTimeout(() => {
        if (overlayMessage) overlayMessage.textContent = '⭐ أحسنت!" هل أنت مستعد للمستوى التالي؟';
        if (successOverlay) successOverlay.classList.remove('hidden');
      }, 1000);
      return;
    }
    ...
    if (gameScore >= 5) {
      ...
      setTimeout(() => {
        ...
        if (nextLvlBtn) nextLvlBtn.classList.add('hidden');
        showOverlay(
          '🏆 اكتمل المستوى 2! لقد طابقت جميع المشاعر الخمسة!',
          'متابعة',
          () => {
            successOverlay.classList.add('hidden');
      ...
      if (choicesGrid) renderChoices(generateChoices(gameTarget.key, 6), choicesGrid);
      resultBox.textContent = `✅ صحيح! ${5 - gameScore} مشاعر${5 - gameScore === 1 ? '' : ''} متبقية.`;
    ...
    resultBox.textContent = `❌ هذا ${selected.label}. حاول مرة أخرى!`;
    resultBox.style.color = '#d00000';
  }
...
function showCongratsScreen() {
  ...
  congrats.innerHTML = `
    ...
    <h2 class="congrats-title">مبروك يا بطل 🏆</h2>
    <img class="congrats-img" src="img/congrats.png" alt="شخصيات احتفالية" />
    <p class="congrats-msg">لقد اتممت جميع المهمات بنجاح ستحصل على أحد الملصقات من مدربك<br></p>
    <button class="start congrats-btn" onclick="window.location.href='index.html'"> العب مرة أخرى</button>
  `;
